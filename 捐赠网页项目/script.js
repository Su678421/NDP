...
const db = {
    ...
    async getMyBeneficiary() {
        ...
        try {
            const response = await fetch(`${API_URL}/api/beneficiary/my`, {
    ...
    async getMessages(otherUsername) {
        ...
        try {
            const response = await fetch(`${API_URL}/api/messages/${otherUsername}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
                return await response.json();
...
async function deleteUser(username) {
    ...
    try {
        ...
        if (usersResponse.ok) {
            ...
            const user = users.find(u => u.username === username);
            if (user && user._id) {
                const deleteResponse = await fetch(API_URL + '/api/admin/users/' + user._id, {
                    method: 'DELETE',
                    headers: { 'Authorization': 'Bearer ' + token }
                });
    ...
    content.innerHTML = '<h3>问题反馈 (' + feedbacks.length + ')</h3>' +
        ...
        feedbacks.map(function(f) {
            ...
            return '<tr>' +
                ...
                '<button onclick="viewFeedbackDetail(\'' + f._id + '\')" style="padding: 5px 10px; background-color: #2196F3; color: #fff; border: none; border-radius: 3px; cursor: pointer; margin-right: 5px;">查看详情</button>' +
                '<button onclick="updateFeedbackStatus(\'' + f._id + '\', \'resolved\')" style="padding: 5px 10px; background-color: #4CAF50; color: #fff; border: none; border-radius: 3px; cursor: pointer;">标记已处理</button>' +
                '<button onclick="deleteFeedback(\'' + f._id + '\')" style="padding: 5px 10px; background-color: #f44336; color: #fff; border: none; border-radius: 3px; cursor: pointer;">删除</button>' +
                '</td>' +
                '</tr>';
        }).join('') +
        '</table>';
...
async function viewFeedbackDetail(feedbackId) {
    ...
    modal.innerHTML = `
        <div style="background-color: #fff; padding: 30px; border-radius: 10px; max-width: 600px; width: 90%; max-height: 80vh; overflow-y: auto;">
            ...
            <div style="margin-bottom: 15px;">
                <strong>提交时间：</strong>${new Date(feedback.createdAt).toLocaleString('zh-CN')}
            </div>
            <div style="margin-bottom: 15px;">
...
async function updateFeedbackStatus(feedbackId, status) {
    const result = await db.updateFeedback(feedbackId, { status });
    if (result) {
        alert('状态已更新');
        loadFeedbacks();
    ...
    modal.innerHTML = `
        ...
        ">
            ...
            <form class="auth-form" onsubmit="event.preventDefault(); handleDonorRegister(this)" style="display: flex; flex-direction: column; gap: 15px;">
                ...
                <input type="tel" placeholder="手机号（11位数字）" required style="padding: 12px; border: 1px solid #ddd; border-radius: 6px; font-family: 'Microsoft YaHei', Arial, sans-serif; font-size: 14px; color: #333;">
                <input type="password" placeholder="密码（6-20个字符）" required style="padding: 12px; border: 1px solid #ddd; border-radius: 6px; font-family: 'Microsoft YaHei', Arial, sans-serif; font-size: 14px; color: #333;">
